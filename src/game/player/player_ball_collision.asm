sub_player_ball_collision:
	...
	ld ix,entities.ball		; dd 21 00 c3 ;1880
	call sub_17f1h_aabb		; cd f1 17 ;1884
	ld (0c082h),bc		; ed 43 82 c0 ;1887
	ld (0c084h),de		; ed 53 84 c0 ;188b
	ld a,(entities.ball.allowed_dirs)		; 3a 12 c3 ;188f
	rrca			; 0f ;1892
	jr c,@check_partners		; 38 0a ;1893
	ld iy,entities.player.1.bottom		; fd 21 00 c2 ;1895
	ld ix,entities.player.2.bottom		; dd 21 80 c2 ;1899
	jr @check_direction		; 18 08 ;189d
@check_partners:
	ld iy,entities.player.1.top		; fd 21 40 c2 ;189f
	ld ix,entities.player.2.top		; dd 21 c0 c2 ;18a3
@check_direction:
...
sub_racket_hit_trajectory:
	bit 7,(ix + entity_t.state_flags)		; dd cb 19 7e ;18b5
	...
	ld a,(ix + entity_t.animation_frame)		; dd 7e 1e ;18c6
	...
	ld a,(ix + entity_t.animation_frame)		; dd 7e 1e ;18df
	add a,a			; 87 ;18e2
	ld e,a			; 5f ;18e3
	ld d,000h		; 16 00 ;18e4
	add hl,de			; 19 ;18e6
	ld a,(ix + entity_t.y_pos + 1)		; dd 7e 0b ;18e7
	add a,(hl)			; 86 ;18ea
	ld b,a			; 47 ;18eb
	inc hl			; 23 ;18ec
	add a,(hl)			; 86 ;18ed
...
@fault_type_3_end:
	...
	res 0,(hl)		; cb 86 ;194f
@hit_success:
	set 7,(ix + entity_t.racket_contact_flag)		; dd cb 1d fe ;1951
	xor a			; af ;1955
...
@miss:
	...
	ld a,(entities.ball.y_pos + 1)		; 3a 0b c3 ;197e
	cp c			; b9 ;1981
	jr c,@no_body_collision		; 38 45 ;1982
	cp b			; b8 ;1984
	jr nc,@no_body_collision		; 30 42 ;1985
	ld a,(entities.ball.x_pos + 1)		; 3a 0d c3 ;1987
	cp e			; bb ;198a
