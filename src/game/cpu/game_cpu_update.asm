sub_game_cpu_update:
	ld a,(ix + player_t.side_state)		; dd 7e 01 ;1e25
	and 003h		; e6 03 ;1e28
	ld e,a			; 5f ;1e2a
	...
	push hl			; e5 ;1e36
	pop iy		; fd e1 ;1e37
	ld a,(ix + player_t.cpu_state)		; dd 7e 30 ;1e39
	and 00fh		; e6 0f ;1e3c
...
partner_offset_table:
	.DB player.2.bottom - player		;1e5a
	.DB player.2.top - player		;1e5b
	.DB player.1.bottom - player		;1e5c
	.DB player.1.top - player		;1e5d

sub_cpu_state_track_ball:
	ld a,(ix + player_t.ball_incoming)		; dd 7e 27 ;1e5e
	and a			; a7 ;1e61
	jr nz,@l1e83h		; 20 1f ;1e62
	ld (ix + player_t.shot_button),a		; dd 77 28 ;1e64
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;1e67
	ld a,(iy+033h)		; fd 7e 33 ;1e6a
	cp 004h		; fe 04 ;1e6d
	jr nz,@l1e75h		; 20 04 ;1e6f
	ld (iy+033h),003h		; fd 36 33 03 ;1e71
@l1e75h:
	ld a,(ix + player_t.cpu_state_timer)		; dd 7e 31 ;1e75
	and 007h		; e6 07 ;1e78
	cp 007h		; fe 07 ;1e7a
	ret nz			; c0 ;1e7c
	ld (ix + player_t.cpu_state),004h		; dd 36 30 04 ;1e7d
...
@l1e83h:
	...
	ld (ix + player_t.cpu_state),006h		; dd 36 30 06 ;1e95
	jr @l1ef0h		; 18 55 ;1e99
@l1e9bh:
	bit 0,(ix + player_t.side_state)		; dd cb 01 46 ;1e9b
	jr nz,@l1eb3h		; 20 12 ;1e9f
	ld a,(0c30bh)		; 3a 0b c3 ;1ea1
	add a,008h		; c6 08 ;1ea4
	ld e,(ix + player_t.y_pos + 1)		; dd 5e 0b ;1ea6
	sub e			; 93 ;1ea9
...
@l1eb3h:
	...
	sub 008h		; d6 08 ;1eb6
	ld e,(ix + player_t.y_pos + 1)		; dd 5e 0b ;1eb8
	sub e			; 93 ;1ebb
...
@l1ec2h:
	...
	ld h,(ix + player_t.x_pos + 1)		; dd 66 0d ;1ed3
	sub h			; 94 ;1ed6
...
@l1edfh:
	...
	ld (ix + player_t.cpu_state),002h		; dd 36 30 02 ;1ee3
	ret			; c9 ;1ee7
@l1ee8h:
	xor a			; af ;1ee8
	ld (ix + player_t.shot_button),a		; dd 77 28 ;1ee9
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;1eec
	ret			; c9 ;1eef
@l1ef0h:
...
@l1ef5h:
	...
	ld (ix + player_t.cpu_state),00ah		; dd 36 30 0a ;1ef9
	ret			; c9 ;1efd
sub_cpu_state_serve_wait:
	ld a,(0c400h)		; 3a 00 c4 ;1efe
	and a			; a7 ;1f01
	ret nz			; c0 ;1f02
	ld a,(ix + player_t.cpu_sub_state)		; dd 7e 33 ;1f03
	cp 002h		; fe 02 ;1f06
	...
	ld (ix + player_t.cpu_state),009h		; dd 36 30 09 ;1f19
	ld (ix + player_t.cpu_sub_state),004h		; dd 36 33 04 ;1f1d
	ret			; c9 ;1f21
@l1f22h:
	ld a,(ix + player_t.ball_incoming)		; dd 7e 27 ;1f22
	and a			; a7 ;1f25
	jr nz,@l1f41h		; 20 19 ;1f26
	ld (ix + player_t.shot_button),a		; dd 77 28 ;1f28
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;1f2b
	ld a,(ix + player_t.cpu_state_timer)		; dd 7e 31 ;1f2e
	and 003h		; e6 03 ;1f31
	cp 003h		; fe 03 ;1f33
	ret nz			; c0 ;1f35
	...
	ret nz			; c0 ;1f3b
@l1f3ch:
	ld (ix + player_t.cpu_state),005h		; dd 36 30 05 ;1f3c
	ret			; c9 ;1f40
@l1f41h:
	...
	ld a,(ix + player_t.cpu_sub_state)		; dd 7e 33 ;1f51
	and a			; a7 ;1f54
	ret z			; c8 ;1f55
...
@l1f68h:
	ld (ix + player_t.cpu_state),b		; dd 70 30 ;1f68
	ret			; c9 ;1f6b
sub_cpu_state_net:
	ld a,(ix + player_t.ball_incoming)		; dd 7e 27 ;1f6c
	and a			; a7 ;1f6f
	jr nz,@l1f7fh		; 20 0d ;1f70
	ld (ix + player_t.cpu_state),000h		; dd 36 30 00 ;1f72
	ld (ix + player_t.cpu_sub_state),000h		; dd 36 33 00 ;1f76
	ld (ix + player_t.shot_button),000h		; dd 36 28 00 ;1f7a
	ret			; c9 ;1f7e
@l1f7fh:
	bit 0,(ix + player_t.side_state)		; dd cb 01 46 ;1f7f
	jr nz,@l1f97h		; 20 12 ;1f83
	ld a,(0c30bh)		; 3a 0b c3 ;1f85
	add a,008h		; c6 08 ;1f88
	ld e,(ix + player_t.y_pos + 1)		; dd 5e 0b ;1f8a
	sub e			; 93 ;1f8d
...
@l1f97h:
	...
	sub 008h		; d6 08 ;1f9a
	ld e,(ix + player_t.y_pos + 1)		; dd 5e 0b ;1f9c
	sub e			; 93 ;1f9f
...
@l1fb5h:
	ld (ix + player_t.cpu_state),a		; dd 77 30 ;1fb5
	ld (iy+033h),b		; fd 70 33 ;1fb8
	ld (ix + player_t.shot_button),000h		; dd 36 28 00 ;1fbb
	ret			; c9 ;1fbf
@l1fc0h:
	...
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;1fce
	call sub_game_cpu_evaluate_y_dist_1		; cd 20 23 ;1fd1
	ld a,(ix + player_t.hit_readiness)		; dd 7e 18 ;1fd4
	and a			; a7 ;1fd7
	ld (ix + player_t.shot_button),a		; dd 77 28 ;1fd8
	ret z			; c8 ;1fdb
	inc (ix + player_t.cpu_state_timer)		; dd 34 31 ;1fdc
	ret			; c9 ;1fdf
@l1fe0h:
	...
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;1fee
	call sub_game_cpu_evaluate_y_dist_2		; cd aa 23 ;1ff1
	ld a,(ix + player_t.hit_readiness)		; dd 7e 18 ;1ff4
	and a			; a7 ;1ff7
	ld (ix + player_t.shot_button),a		; dd 77 28 ;1ff8
	ret z			; c8 ;1ffb
@l1ffch:
	inc (ix + player_t.cpu_state_timer)		; dd 34 31 ;1ffc
	ret			; c9 ;1fff
sub_cpu_state_baseline_attack:
	ld a,(ix + player_t.ball_incoming)		; dd 7e 27 ;2000
	and a			; a7 ;2003
	jr nz,@l2013h		; 20 0d ;2004
	ld (ix + player_t.cpu_state),009h		; dd 36 30 09 ;2006
	ld (ix + player_t.cpu_sub_state),000h		; dd 36 33 00 ;200a
	ld (ix + player_t.shot_button),000h		; dd 36 28 00 ;200e
	ret			; c9 ;2012
@l2013h:
	bit 0,(ix + player_t.side_state)		; dd cb 01 46 ;2013
	...
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;2027
	call sub_game_cpu_evaluate_y_dist_1		; cd 20 23 ;202a
	ld a,(ix + player_t.hit_readiness)		; dd 7e 18 ;202d
	ld (ix + player_t.shot_button),a		; dd 77 28 ;2030
	inc (ix + player_t.cpu_state_timer)		; dd 34 31 ;2033
	ret			; c9 ;2036
@l2037h:
	...
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;2045
	call sub_game_cpu_evaluate_y_dist_2		; cd aa 23 ;2048
	ld a,(ix + player_t.hit_readiness)		; dd 7e 18 ;204b
	ld (ix + player_t.shot_button),a		; dd 77 28 ;204e
	inc (ix + player_t.cpu_state_timer)		; dd 34 31 ;2051
	ret			; c9 ;2054
sub_cpu_state_return_center:
	bit 0,(ix + player_t.ball_incoming)		; dd cb 27 46 ;2055
	jr nz,@l207eh		; 20 23 ;2059
	ld a,(ix+02eh)		; dd 7e 2e ;205b
	bit 0,(ix + player_t.side_state)		; dd cb 01 46 ;205e
	jr z,@l2075h		; 28 11 ;2062
	cp 002h		; fe 02 ;2064
	jr c,@l207eh		; 38 16 ;2066
	ld (ix + player_t.input_dirs),001h		; dd 36 15 01 ;2068
	ld (ix + player_t.shot_button),000h		; dd 36 28 00 ;206c
	ld (iy+033h),005h		; fd 36 33 05 ;2070
	ret			; c9 ;2074
@l2075h:
	cp 0b0h		; fe b0 ;2075
	jr nc,@l207eh		; 30 05 ;2077
	ld (ix + player_t.input_dirs),002h		; dd 36 15 02 ;2079
	ret			; c9 ;207d
@l207eh:
	xor a			; af ;207e
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;207f
	ld (ix + player_t.cpu_state_timer),a		; dd 77 31 ;2082
	ld (ix + player_t.cpu_state),003h		; dd 36 30 03 ;2085
...
sub_cpu_state_wait:
	...
	and a			; a7 ;2091
	ret nz			; c0 ;2092
	bit 0,(ix + player_t.ball_incoming)		; dd cb 27 46 ;2093
...
@l20a5h:
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;20a5
	ld a,(ix+02eh)		; dd 7e 2e ;20a8
	cp 006h		; fe 06 ;20ab
	ret c			; d8 ;20ad
	ld (ix + player_t.cpu_sub_state),000h		; dd 36 33 00 ;20ae
@l20b2h:
	ld a,(0c000h)		; 3a 00 c0 ;20b2
	bit 7,a		; cb 7f ;20b5
	jr z,@l20c0h		; 28 07 ;20b7
	ld a,(ix + player_t.cpu_sub_state)		; dd 7e 33 ;20b9
	cp 005h		; fe 05 ;20bc
	jr z,@l2099h		; 28 d9 ;20be
@l20c0h:
	xor a			; af ;20c0
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;20c1
	ld (ix + player_t.cpu_state_timer),a		; dd 77 31 ;20c4
	ld (ix + player_t.cpu_state),000h		; dd 36 30 00 ;20c7
	ret			; c9 ;20cb
sub_cpu_state_baseline_defense:
	ld a,(ix + player_t.ball_incoming)		; dd 7e 27 ;20cc
	and a			; a7 ;20cf
	jr nz,@l20dfh		; 20 0d ;20d0
	ld (ix + player_t.cpu_state),004h		; dd 36 30 04 ;20d2
	ld (ix + player_t.shot_button),000h		; dd 36 28 00 ;20d6
	ld (iy+033h),005h		; fd 36 33 05 ;20da
	ret			; c9 ;20de
@l20dfh:
	bit 0,(ix + player_t.side_state)		; dd cb 01 46 ;20df
	...
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;20f3
	call sub_game_cpu_evaluate_y_dist_1		; cd 20 23 ;20f6
	ld a,(ix + player_t.hit_readiness)		; dd 7e 18 ;20f9
	and a			; a7 ;20fc
	ret z			; c8 ;20fd
	ld (ix + player_t.shot_button),a		; dd 77 28 ;20fe
...
@l2106h:
	...
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;2114
	call sub_game_cpu_evaluate_y_dist_2		; cd aa 23 ;2117
	ld a,(ix + player_t.hit_readiness)		; dd 7e 18 ;211a
	and a			; a7 ;211d
	ret z			; c8 ;211e
	ld (ix + player_t.shot_button),a		; dd 77 28 ;211f
	ret			; c9 ;2122
sub_cpu_state_doubles_cover_1:
	bit 7,(ix + player_t.cpu_state)		; dd cb 30 7e ;2123
	jr nz,@l2132h		; 20 09 ;2127
	set 7,(ix + player_t.cpu_state)		; dd cb 30 fe ;2129
	ld (ix + player_t.shot_button),000h		; dd 36 28 00 ;212d
	ret			; c9 ;2131
@l2132h:
	ld a,(ix + player_t.cpu_sub_state)		; dd 7e 33 ;2132
	cp 002h		; fe 02 ;2135
	...
	ld (ix + player_t.cpu_state),003h		; dd 36 30 03 ;2141
	ret			; c9 ;2145
@l2146h:
	ld (ix + player_t.cpu_state),005h		; dd 36 30 05 ;2146
	ret			; c9 ;214a
@l214bh:
...
@l2176h:
	ld (ix + player_t.input_dirs),b		; dd 70 15 ;2176
	ret			; c9 ;2179
@l217ah:
	xor a			; af ;217a
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;217b
	ld (ix + player_t.cpu_state),001h		; dd 36 30 01 ;217e
	ret			; c9 ;2182
...
@l21aeh:
	ld (ix + player_t.input_dirs),b		; dd 70 15 ;21ae
	ret			; c9 ;21b1
@l21b2h:
	xor a			; af ;21b2
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;21b3
	ld (ix + player_t.cpu_state),000h		; dd 36 30 00 ;21b6
	ret			; c9 ;21ba
sub_cpu_state_idle_wander:
	bit 7,(ix + player_t.cpu_state)		; dd cb 30 7e ;21bb
	jr nz,@l21cdh		; 20 0c ;21bf
	set 7,(ix + player_t.cpu_state)		; dd cb 30 fe ;21c1
	ld a,r		; ed 5f ;21c5
	and 01fh		; e6 1f ;21c7
	inc a			; 3c ;21c9
	ld (ix + player_t.cpu_state_timer),a		; dd 77 31 ;21ca
@l21cdh:
	dec (ix + player_t.cpu_state_timer)		; dd 35 31 ;21cd
	ret nz			; c0 ;21d0
...
@l21e4h:
	and 004h		; e6 04 ;21e4
@l21e6h:
	ld (ix + player_t.input_dirs),a		; dd 77 15 ;21e6
	res 7,(ix + player_t.cpu_state)		; dd cb 30 be ;21e9
	ret			; c9 ;21ed
sub_cpu_state_smash:
	bit 7,(ix + player_t.cpu_state)		; dd cb 30 7e ;21ee
	jr nz,@l21fch		; 20 08 ;21f2
	set 7,(ix + player_t.cpu_state)		; dd cb 30 fe ;21f4
	ld (ix + player_t.cpu_state_timer),000h		; dd 36 31 00 ;21f8
@l21fch:
	ld a,(0c501h)		; 3a 01 c5 ;21fc
	cp 02ah		; fe 2a ;21ff
	jr c,@l2207h		; 38 04 ;2201
	set 0,(ix + player_t.cpu_state_timer)		; dd cb 31 c6 ;2203
@l2207h:
	bit 0,(ix + player_t.cpu_state_timer)		; dd cb 31 46 ;2207
	ret z			; c8 ;220b
	cp 028h		; fe 28 ;220c
	ret nc			; d0 ;220e
	cp 01ah		; fe 1a ;220f
	...
	and 001h		; e6 01 ;2216
	ret nz			; c0 ;2218
	ld (ix + player_t.shot_button),001h		; dd 36 28 01 ;2219
	ret			; c9 ;221d
