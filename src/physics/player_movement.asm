sub_player_movement:
	...
	ld a,(ix + entity.type)		; dd 7e 02 ;26d4
...
+:
	...
	ld a,(ix + entity.side_state)		; dd 7e 01 ;26e2
	and 1		; e6 01 ;26e5
	jr nz,+		; 20 0c ;26e7
...
+:
	...
	jr ++		; 18 0a ;2702
l2704h:
	ld a,(ix + entity.side_state)		; dd 7e 01 ;2704
	and 1		; e6 01 ;2707
	jr z,++		; 28 03 ;2709
	ld hl,BOUND_Y_MIN_DEFAULT		; 21 80 36 ;270b
++:
	ld (ix + entity.cache_y_min),l		; dd 75 3c ;270e
	ld (ix + entity.cache_y_min + 1),h		; dd 74 3d ;2711
	ld d,(ix + entity.y_pos + 1)		; dd 56 0d ;2714
	ld a,(ix + entity.y_pos)		; dd 7e 0c ;2717
...
+:
	...
	ld a,(ix + entity.type)		; dd 7e 02 ;2730
...
+:
	...
	ld a,(ix + entity.side_state)		; dd 7e 01 ;273e
	and 1		; e6 01 ;2741
	jr nz,+		; 20 0c ;2743
...
+:
	...
	jr ++		; 18 0a ;275e
l2760h:
	ld a,(ix + entity.side_state)		; dd 7e 01 ;2760
	and 1		; e6 01 ;2763
	jr z,++		; 28 03 ;2765
	ld hl,BOUND_Y_MAX_DEFAULT		; 21 ff c9 ;2767
++:
	ld (ix + entity.cache_y_max),l		; dd 75 3e ;276a
	ld (ix + entity.cache_y_max + 1),h		; dd 74 3f ;276d
	ld d,(ix + entity.y_pos + 1)		; dd 56 0d ;2770
	ld a,(ix + entity.y_pos)		; dd 7e 0c ;2773
	sub l			; 95 ;2776
...
++:
	...
	ld a,(ix + entity.type)		; dd 7e 02 ;278f
	and ENT_TYPE_MASK		; e6 7f ;2792
	cp b			; b8 ;2794
	jr z,l27e0h		; 28 49 ;2795
	cp c			; b9 ;2797
	jr z,l27e0h		; 28 46 ;2798
	ld a,(ix + entity.side_state)		; dd 7e 01 ;279a
	and 1		; e6 01 ;279d
	jr z,+		; 28 03 ;279f
	ld hl,BOUND_X_MIN_LEFT_HALF		; 21 00 1d ;27a1
+:
	ld d,(ix + entity.x_pos + 1)		; dd 56 0b ;27a4
	ld a,(ix + entity.x_pos)		; dd 7e 0a ;27a7
...
+:
	...
	ld a,(ix + entity.side_state)		; dd 7e 01 ;27bc
	and 1		; e6 01 ;27bf
	jr z,+		; 28 03 ;27c1
	ld hl,BOUND_X_MAX_LEFT_HALF		; 21 ff 4c ;27c3
+:
	ld d,(ix + entity.x_pos + 1)		; dd 56 0b ;27c6
	ld a,(ix + entity.x_pos)		; dd 7e 0a ;27c9
	sub l			; 95 ;27cc
	jr nc,+		; 30 06 ;27cd
	ld a,d			; 7a ;27cf
	sub 1		; d6 01 ;27d0
...
l27e4h:
	ld a,(ix + entity.tile_collision)		; dd 7e 2d ;27e4
	and DIR_MASK		; e6 0f ;27e7
	ld b,a			; 47 ;27e9
	ex af,af'			; 08 ;27ea
	or b			; b0 ;27eb
	cpl			; 2f ;27ec
	ld b,(ix + entity.input_dirs)		; dd 46 15 ;27ed
	and b			; a0 ;27f0
	ld (ix + entity.allowed_dirs),a		; dd 77 12 ;27f1
	ret			; c9 ;27f4
